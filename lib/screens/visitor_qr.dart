    ...
    try {
      ...
      if (data['success'] == true) {
        setState(() {
          ...
          _ttl = data['expiresInSeconds'] is int
        ...
        if (!mounted) return;
        final msg =
            (data['message'] ?? 'Failed to fetch QR token').toString();
      ...
      final msg = e.response?.data is Map
          ? ((e.response?.data['message'] ?? 'Failed to fetch QR token')
              .toString())
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to fetch QR token')));
    } finally {
